...
get_brightness_external() {
    pkexec ddcutil getvcp 10 | awk '{print $9}' | tr -d ',\n'
}
...
set_brightness_external() {
    pkexec ddcutil setvcp 10 $1
}

fade_brightness_internal() {
    if [[ -z $sysfs_path ]]; then
        xbacklight -time $fade_time -steps $fade_steps -set $1
    elif [[ -z $fade_step_time ]]; then
        set_brightness_internal $1
    else
        local level
        for level in $(eval echo {$(get_brightness_internal)..$1}); do
            set_brightness_internal $level
...
trap "set_brightness_internal $(get_brightness_internal); set_brightness_external $(get_brightness_external); kill %%" EXIT
fade_brightness_internal $min_brightness
set_brightness_external $min_brightness
sleep 2147483647 &
wait
