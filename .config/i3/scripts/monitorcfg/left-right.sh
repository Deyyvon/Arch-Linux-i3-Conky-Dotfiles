...
# Laptop is on the left, external monitor is on the right.

LEFT_DISPLAY='HDMI1'	# Left screen from xrandr
...
if [[ $LEFT_STATUS == 'connected' && $RIGHT_STATUS == 'connected' ]]; then
    ...
    mvi3ws $RIGHT_DISPLAY $LEFT_DISPLAY

    # Put polybar on appropriate monitors
    pbar $LEFT_DISPLAY $RIGHT_DISPLAY

# If only left connected, use it and internal display.
elif [[ $LEFT_STATUS == 'connected' ]]; then
    xrandr --output $INT_DISPLAY --left-of $LEFT_DISPLAY --primary --auto
...
# If only right connected, use it and internal display.
elif [[ $RIGHT_STATUS == 'connected' ]]; then
    xrandr --output $INT_DISPLAY --left-of $RIGHT_DISPLAY --primary --auto
